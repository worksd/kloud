...
// 레이아웃: 상단 cover 배너 + 2-column (좌: 배너/타임테이블/공지/YouTube/수업 그리드, 우: sticky SNS + 패스권 구매)
// (shelf 'Add PC-specific components' 이식분 — WebTopNav가 sticky(문서 흐름)라 상단 오프셋만 조정)
          ...
          <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between gap-4">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="flex flex-col gap-1 min-w-0">
                ...
                {studio.address && (
                  <span className="text-[13px] font-medium text-white/90 truncate">{studio.address}</span>
                )}
              </div>
            </div>
      ...
      <div className="mx-auto w-full max-w-6xl px-8 mt-10 grid grid-cols-[minmax(0,1fr)_340px] gap-x-10">
        ...
        <div className="col-start-1 row-start-1 flex flex-col gap-10 min-w-0">
          ...
          {studio.banners && studio.banners.length > 0 && (
            <section>
              {/* 배너 한 장이 컬럼 폭을 꽉 채우고, 여러 장이면 스와이프(snap)로 넘긴다.
          ...
          {/* 진행중인 수업 — PC 자체 3-column 그리드 (모바일 LessonGridSection은 w-screen 기반이라 PC 부적합) */}
          {lessons.length > 0 && (
            <section>
        ...
        {/* 우측 sticky 카드 — 패스권 구매 CTA (SNS는 커버 배너 우하단으로 이동).
            top-20: sticky 상단바(h-16) 아래 여백 확보 */}
        <aside className="col-start-2 row-start-1 sticky top-20 self-start">
          <div className="rounded-2xl border border-[#f0f1f3] bg-white p-5 shadow-sm">
