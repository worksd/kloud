...
const LessonInfoSection = ({data}: {data: GetLessonResponse}) => {
    ...
    const startTime = formatDateTime(data.startTime ?? '', locale);
    ...
    return <div className="self-stretch px-6 py-5 flex-col justify-start items-start gap-4 flex">
        <div className="self-stretch flex-col justify-start items-start gap-2 flex">
            ...
            <LessonInfoLabel Icon={Calendar} text={`${startTime.date}(${startTime.dayOfWeek})`} subText={calculateDDays(data.startTime ?? '') ?? t('finish')} />

            <LessonInfoLabel Icon={TimeCircle} text={startTime.time} subText={`${data.duration} ${t('minutes')}`} />
