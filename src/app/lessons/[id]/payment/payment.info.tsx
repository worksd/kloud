    ...
    <div>
      ...
      <div className="flex flex-col gap-y-4 px-6">
        ...
        <div className="flex flex-col gap-y-2">
          <div className="flex justify-between text-sm font-medium text-center text-black">
            <p>{t('lesson_price')}</p>
            <p>{new Intl.NumberFormat("ko-KR").format(lesson.price ?? 0)} {t('won')}</p>
          </div>
      ...
      <div className="flex flex-col gap-y-5 px-6">
        ...
        <DropdownDetails title={t('seller_information')}>
          <SellerInfoItem label={t('business_name')} value={lesson.studio?.businessName ?? ''}/>
          {lesson.studio?.businessRegistrationNumber &&
            <SellerInfoItem label={t('business_registration_number')} value={lesson.studio.businessRegistrationNumber}/>}
          {lesson.studio?.eCommerceRegNumber &&
            <SellerInfoItem label={t('e_commerce_registration_number')} value={lesson.studio.eCommerceRegNumber}/>}
          {lesson.studio?.representative && <SellerInfoItem label={t('representative')} value={lesson.studio.representative}/>}
          {lesson.studio?.educationOfficeRegNumber &&
            <SellerInfoItem label={t('education_office_registration_number')} value={lesson.studio.educationOfficeRegNumber}/>}
          {lesson.studio?.address && <SellerInfoItem label={t('business_address')} value={lesson.studio.address}/>}
        </DropdownDetails>
