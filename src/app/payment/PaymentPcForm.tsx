// PC 전용 결제 페이지 렌더 — 모바일/앱 웹뷰는 page.tsx의 기존 인라인 렌더 사용.
// 2-column 레이아웃 + 우측 sticky 아이템 요약 카드. 분기는 page.tsx에서 appVersion + viewport(lg)로.
// (shelf 'Add PC-specific components' 이식분 — 이후 추가된 매주 요일(weeklyLabel)·강사 표시를 반영)
...
export default async function PaymentPcForm({
  ...
  locale: Locale;
  apiUrl: string;
  /** 정기(가격 정책) 수업이면 '매주 목요일, 금요일' — page.tsx에서 계산해 모바일과 동일 값을 공유 */
  ...
  return (
    <div className="w-full min-h-screen bg-white pt-12 pb-32">
      <div className="mx-auto w-full max-w-5xl px-8 grid grid-cols-[minmax(0,1fr)_360px] gap-x-12">
        ...
        <aside className="col-start-2 row-start-1 sticky top-12 self-start">
          <div className="rounded-2xl border border-[#f0f1f3] bg-white p-5 shadow-sm flex flex-col gap-4">

            {/* lesson — PC에서는 큰 썸네일 위 + 메타 아래 세로 배치 */}
            {paymentItem === 'lesson' && (
              <>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {(payment.lesson?.formattedDate || payment.lesson?.date || weeklyLabel) && (
                    <div className="flex flex-col gap-1 mt-1">
                      ...
                      {(payment.lesson?.formattedDate?.startTime || payment.lesson?.duration || (weeklyLabel && payment.lesson?.date)) && (
                        <div className="flex items-center gap-1.5">
                          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                          ...
                          <span className="text-[13px] font-medium text-[#666]">
                            {payment.lesson?.formattedDate
                              ? `${weeklyLabel && payment.lesson.formattedDate.date
                                  ? `${payment.lesson.formattedDate.date}${payment.lesson.formattedDate.weekday ? ` (${payment.lesson.formattedDate.weekday})` : ''} `
                                  : ''}${payment.lesson.formattedDate.startTime} - ${payment.lesson.formattedDate.endTime}`
                              : weeklyLabel && payment.lesson?.date
                                // 정기 수업 실응답엔 formattedDate가 없다 — BE 완성 포맷(date)이 시작일+시각
                                ? payment.lesson.date
                                : `${payment.lesson?.duration}${await translate('minutes')}`}
                          </span>
                        </div>
                      )}
                      {/* 강사 — 프로필 사진 + 이름 (모바일 렌더와 동일 규칙: 닉네임 우선) */}
                      {payment.lesson?.artists && payment.lesson.artists.length > 0 && (
                        <div className="flex flex-col gap-1.5 mt-0.5">
                          {payment.lesson.artists.map((artist) => {
          ...
          {/* 결제 버튼 슬롯 — UnifiedPaymentInfo가 여기로 버튼을 portal한다 (sticky aside라 스크롤해도 따라옴) */}
          <div id={PC_PAYMENT_BUTTON_SLOT_ID} className="mt-4"/>
        </aside>
