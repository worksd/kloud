...
const CommonSubmitButton = ({ children, disabled, originProps }: IProps) => {
    ...
    return (
    <button className={`left flex justify-center font-bold items-center w-full h-14 rounded-lg active:scale-[0.95] transition-transform duration-150 select-none ${isDisable ? "bg-[#bcbfc2]" : "bg-black"}`} {...originProps}>
        {children}
    </button>
)};
