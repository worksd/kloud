...
export const MembershipPaymentInfo = ({payment, price, url, appVersion, beforeDepositor, locale, membershipPlanName}: {
  ...
  return (
    <div className={"flex flex-col justify-between"}>
      ...
      <div className="flex flex-col gap-y-5 px-6">
        {/* 판매자 정보 */}
        {payment.membershipPlan?.studio && (
          <SellerInformation 
            studio={payment.membershipPlan.studio} 
            locale={locale}
          />
        )}
        {!payment.membershipPlan?.studio && payment.passPlan?.studio && (
          <SellerInformation 
            studio={payment.passPlan.studio} 
            locale={locale}
          />
        )}
        {/* 환불 안내 */}
